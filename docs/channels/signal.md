...
- **`ReplyToId`**: Quoted message ID (or timestamp as fallback).
...
Format: `Shared contact: <name> (<first-phone>, <first-email>, <organization>)`
...
- **Poll votes**: option indexes
  - Format: `Poll vote on #<timestamp>: option(s) <indexes>`
