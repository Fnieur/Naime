...
describe("BlockReplyCoalescer hold/resume behavior", () => {
  ...
  it("buffers flushOnEnqueue payloads while held and flushes on resume", async () => {
    const onFlush = vi.fn();
    const coalescer = createBlockReplyCoalescer({
      config: { minChars: 1, maxChars: 1000, idleMs: 1000, joiner: "\n\n", flushOnEnqueue: true },
      shouldAbort: () => false,
    ...
    coalescer.hold();
    coalescer.enqueue({ text: "paragraph_1" });
    coalescer.enqueue({ text: "paragraph_2" });
    ...
    expect(onFlush).toHaveBeenCalledTimes(1);
    expect(onFlush).toHaveBeenCalledWith({ text: "paragraph_1\n\nparagraph_2" });
  });
